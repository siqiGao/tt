...
import re
import time
import LambdaRankNNmaster.shishi20 as LambdaRank
#from pyltp import Segmentor, Postagger, Parser, NamedEntityRecognizer
from nltk.metrics.distance import jaccard_distance
import difflib
import config
import Levenshtein
import bert.Attribute.similarity as si
    ...
    for name in entity_list:
        ...
        for entity in entity_total:
            ...
            for relation in relations:  # 不同的关系，可能有类别相同的关系
                ...
                score = serviceWord2vec.get_similarity(list(jieba.cut(temp0)), list(jieba.cut(relation['name'])))  # 只要实体关系和句子沾边
            ...
            p.close()
            '''
            entity_info = [simple_name, entity_Id, name_simi_score, in_question_word, max_relation_score, relation_in_question,
...
def get_realtion_info(relation_candidate, question):  # [name, relation, target_entity, target_entity_keyid]
    ...
    for candidate in relation_candidate:
        if '/' in candidate[0]:
            entity_name = candidate[0].split('/')[0] + '的' + candidate[0].split('/')[1]
        else:
        ...
        w2v = simi(question, entity_name + '的' +candidate[1])
        ...
        if candidate[2] != candidate[0]:  #防止出现扰乱关系和实体成环现象
            relation_info.append(
                [candidate[0], candidate[1], candidate[2], candidate[3], candidate[4], in_question_word, relation_in_question,word_jaccard, words_jaccard,
                 leve_score, edit, w2v])
        ...
        for i in range(len(result2_10)):
            ...
            if result2_10[i][2] in temp_queston:
                ...
                if temp_score > max_score:
                    ...
                    result3.append([entity,relation1+'*',onehopentity, 0, ['Instance'],result2_10[i][1] , result2_10[i][2]])
...
def choose_type(result1, result2, question):
    ...
    #time.sleep(10)
    if (type_score1+1 < type_score2 and result2[0][-1] >= 0.2) or (type_score1 < type_score2 and result2[0][-1] >= 0.6):
        result4 = result2
    else:
        result4 = result1
    #print(result4)
...
def answer(line):
    ...
    #print(word_list)
    entity_list = entityRecognize(word_list, question)  # 这里召回的是指称
    #print(entity_list)
    ...
    try:
        ...
        #print(neirong)
    q = open("answer_84.txt", 'a', encoding="utf-8")
