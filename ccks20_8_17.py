...
def entityLink(entity_list, question):  # (通过实体名找到数据库中的各实体并通过评分策略找到中心实体)
    ...
    for name in entity_list:
        ...
        for entity in entity_total:
            ...
            p.close()
            '''
            entity_info = [simple_name, entity_Id, name_simi_score, in_question_word, max_relation_score,
...
########################################################################
def reverse_discription(question):
    bert_list = bert_ner(question)
    results = []
    last = []
    if len(bert_list) == 1:
        discription = bert_list[0]
        results = ccksNeo.get_entity_by_discription(discription)
        ...
        if results:
            ...
            for result in results:
                ...
                if simi_score > max_score:
                    result.append(simi_score)
                    last = [discription] + result
                elif simi_score == max_score:
                    result.append(simi_score)
                    last.append([discription] + result)
...
def template(using_entity, question):  # 使用实体id，传进来的是[name , keyid]
    ...
    if result3 != []:
        ...
        reverse_dis = reverse_discription(question)
        if reverse_dis and result4[0][-1] < 0.9 and reverse_dis[0][-1] > result4[0][-1]:
            return reverse_dis
        else:
            return result4
    '''
...
def answer(line):
    ...
    try:
        ...
        # print(neirong)
    q = open("answer_817.txt", 'a', encoding="utf-8")
