...
def get_entity_by_discription(entity):
    #MATCH (n:Instance)-[r]->(m:Discription) where m.name="天下第一阳刚神功" return n,r,m limit 10
    statement = "match (n:Instance)-[r]->(m:Discription) where m.name=\"" + str(entity) + "\" return r.value,n.name,id(n),labels(n) limit 10"
    result = execute(statement)
    entity = []
    if len(result) != 0:
        for i in result:
